    ...
    def transform(self, df, phase):
        ...
        for name, group in groups.items():
            if name in self.aggs:
                ...
                rename_dict = {
                    k: f"fe_group_key_ID_last{name}_sub_{k}" for k in self.feats
                }
...
class GroupbyIDLastDivTransformer:
    def __init__(self, feats, aggs):
        self.feats = feats
        self.aggs = aggs
    ...
    def transform(self, df, phase):
        ...
        for name, group in groups.items():
            if name in self.aggs:
                ...
                rename_dict = {
                    k: f"fe_group_key_ID_last{name}_div_{k}" for k in self.feats
                }
                group_last_div = group_last_div.rename(columns=rename_dict)

                # 0除算の場合infになるため、エラー回避のためにnanをセットする
                for c in [f for f in group_last_div.columns if f != "customer_ID"]:
                    group_last_div[c] = group_last_div[c].astype(float)
                    group_last_div.loc[np.isinf(group_last_div[c]), c] = np.nan
